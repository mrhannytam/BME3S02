...
def difficulty():
    while True:
        ...
        button("簡單", EASY_BUTTON_POSITION_X, DISPLAY_HEIGHT/1.5, 120, 50, GREEN, BRIGHT_GREEN, game_loop, 'easy')
        button("困難", HARD_BUTTON_POSITION_X, DISPLAY_HEIGHT/1.5, 120, 50, RED, BRIGHT_RED, game_loop, 'hard')
...
def game_loop(difficult = None):
    ...
    while True:
        ...
        #Show the time counting
        timeSurf, timeRect = text_objects(time, font)
        timeRect.center = (DISPLAY_WIDTH / 2), (DISPLAY_HEIGHT/6)
        screen.blit(timeSurf, timeRect)

        #Show the question
        questSurf, questRect = text_objects(quest_text, font)
        questRect.center = (DISPLAY_WIDTH / 2), (DISPLAY_HEIGHT/4)
        screen.blit(questSurf, questRect)

        #Show the question image
        quest_image = pygame.image.load(CURRENT_QUESTION_IMAGE)
        screen.blit(quest_image,((DISPLAY_WIDTH / 5), (DISPLAY_HEIGHT/2)))
